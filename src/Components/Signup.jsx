...
import { signupUser } from "../Helpers/helper";
...
export default function Signup(){
    ...
    return(
        <form className=" text-center" onSubmit={handleSubmit}>
              ...
              <input value={values.name} name="name" onChange={handleChange} onBlur={handleBlur} type="text" className="grow" placeholder="Username"/>
              </label>
              {touched.name && errors.name?(<div className="font-bold text-xs mb-4">{errors.name}</div>):("")}
              <label className="input input-bordered flex items-center gap-2">
              ...
              <button className="btn" type="submit">{loading==="off"?"Signup":<span className="loading loading-spinner loading-xs"></span>}</button>
              {result?<h3>{result}</h3>:""}
        </form>
    )
}